...
{ 
  ...
  {
    case 0:MPU6050_DEBUG("MPU6050 not found...\r\n");
      break;
    case 1:MPU6050_DEBUG("MPU6050 check success...\r\n");
      break;
  }
} 
/**************************实现函数********************************************
...
uint8_t MPU6050_getIntStatus(void) {
    return IICreadByteOne(devAddr, MPU6050_RA_INT_STATUS);
}
...
uint8_t MPU6050_getOTPBankValid(void) {
	uint8_t  temp = IICreadByteOne(devAddr, MPU6050_RA_XG_OFFS_TC);
...
int8_t MPU6050_getYGyroOffsetTC(void) {
	uint8_t  temp = IICreadByteOne(devAddr, MPU6050_RA_YG_OFFS_TC);
	temp &= 0x3F;
    return temp;
}
...
int8_t MPU6050_getZGyroOffsetTC(void) {
	uint8_t  temp = IICreadByteOne(devAddr, MPU6050_RA_ZG_OFFS_TC);
	temp &= 0x3F;
    return temp;
}
